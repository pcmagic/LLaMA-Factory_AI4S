...
def do_tokenization(
    ...
    dataset = load_single_dataset(dataset_attr, model_args, data_args, stage)
    if dataset is None:
        tinfo = "ignore wrong dataset:" + str(dataset_attr)
        logger.warning(tinfo)
        return dataset
    else:
        ...
        if not data_args.streaming:
            logger.info(
                f"Total number of data files after tokenizer: {len(dataset)}\n\n\n"
            )
        tokenized_path = os.path.join(
            data_args.tokenized_path,
            os.path.basename(os.path.splitext(dataset_attr.dataset_name)[0]),
        )
        dataset = Dataset.from_list(list(dataset))
        dataset.save_to_disk(tokenized_path, max_shard_size="1GB")
        logger.info("Tokenized dataset saved at {}.".format(tokenized_path))
        logger.info(
...
def tokenizer_single(args: Optional[Dict[str, Any]] = None) -> None:
    ...
    for dl in ddir:
        if dl.startswith("PRETRAIN_DATA_PATH"):
            dl = os.environ.get("PRETRAIN_DATA_PATH") + dl.replace(
                "PRETRAIN_DATA_PATH", ""
            )
        new_ddir.append(dl)
    ...
    dataset_sort_idx = natsort.index_natsorted(
        [str(i0) for i0 in dataset_attr_array])
    dataset_attr_array = dataset_attr_array[dataset_sort_idx]
    logger.info(dataset_attr_array)

    # single process version
    for dataset_attr in dataset_attr_array:
    ...
    #     tokenizer = load_tokenizer(model_args)["tokenizer"]
    #     do_tokenization(dataset_attr, model_args, data_args, stage, tokenizer)
    # args_list = [(dataset_attr, model_args, data_args, stage)
    #              for dataset_attr in dataset_attr_array]
...
def tokenizer_jsonname(args: Optional[Dict[str, Any]] = None) -> None:
    model_args, data_args, _, finetuning_args, _ = get_train_args(args)
    tokenizer = load_tokenizer(model_args)["tokenizer"]
    logger.info('ignore parameter dataset_dir: %s' % data_args.dataset_dir)
    data_args.dataset = [os.path.abspath(data_args.dataset_file), ]
    data_args.dataset_dir = [os.path.dirname(data_args.dataset[0]), ]
    dataset_attr = get_dataset_list(data_args)[0]
    assert finetuning_args.stage == "pt"
    do_tokenization(dataset_attr, model_args, data_args,
                    finetuning_args.stage, tokenizer,)
